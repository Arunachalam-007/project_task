...
public class BankValidation {
	...
	public String custLocVal(String ccity) {
		String expression = "(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%^&+=])(?=\\S+$).{8,}";
		
		if((ccity.trim().length()>3)&&(ccity.matches(expression))) {
			System.out.println("Customer location is valid");
			return ccity;
		}
		else {
			System.out.println("Invalid Customer location");
			System.exit(1);	
			
		}
		return "plz enter valid customer location";
